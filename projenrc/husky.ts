...
function traverse<Type, Argument>(
  ...
  const _traverse = (o: Dynamic<Type, Argument>): Type => {
    ...
    const isObjectLike = R.is(Object);
    const isNotFunction = R.complement(R.is(Function));
    const isObject = R.both(isObjectLike, isNotFunction);
    const continueTraverse = (v: any) =>
      isArray(v) || isObject(v) ? _traverse(v) : cond(v);
    return (isArray(o)
      ? R.map(continueTraverse, o)
      : R.compose(
          R.mergeAll as any,
          R.map(([k, v]) => R.assoc(k, continueTraverse(v), {})),
          R.toPairs,
        )(o)) as unknown as Type;
  };
